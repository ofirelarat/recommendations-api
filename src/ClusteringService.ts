import { ObjectWithValues, IDataModel, IdType, ValueType, Recommendation, GraphNode } from './types';
...
export class ClusteringService {
     ...
     * @returns {Promise<Recommendation[]>} - The list of most common values with their scores.
     */
    async findMostCommonValues(targetValue: ValueType, numValuesToReturn: number): Promise<Recommendation[]> {
     ...
     * @param {number} numValuesToReturn - The number of recommendations to return.
     * @returns {Promise<Recommendation[]>} - The list of most common values not already associated with the object, with their scores.
     */
    async findMostCommonValuesForId(objectId: IdType, numValuesToReturn: number): Promise<Recommendation[]> {
        ...
        for (const value of targetObject.values) {
            ...
            if (recs) {
                for (const rec of recs) {
                    if (!targetValues.has(rec.value) && !recommendations.some(r => r.value === rec.value)) {
     ...
     * 
     * @returns {Promise<Recommendation[]>} - The list of overall most common values with their scores.
     */
    async findOverallMostCommonValues(): Promise<Recommendation[]> {
        return await this.dataModel.getTopOverallRecommendations();
    }

    /**
     * Get the node by ID
     * @param {IdType} id - The ID of the node to retrieve.
     * @returns {Promise<GraphNode | undefined>}
     */
    async getNode(id: IdType): Promise<GraphNode | undefined> {
        return this.dataModel.getNode(id);
    }
}
